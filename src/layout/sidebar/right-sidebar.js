import { Box, Button, Checkbox, FormControlLabel, Modal, Tooltip } from '@mui/material';
...
import { removeSavedResult, selectOutputResult } from '../../redux/reducer/resultSlice';
              ...
              savedResult.map((result, i) => (
                <div key={i} className='flex justify-between items-center'>
                  ...
                  <Tooltip title={result?.name}>
                   <p className="text-gray-500">{result.name?.slice(0, 12)}{result.name?.length > 12 && "..." }</p>
                  </Tooltip>
                  ...
                  control={<Checkbox
                    icon={<MdOutlineCheckBoxOutlineBlank
                      className='text-[#F24E1E] text-2xl mb-[2px]' />}
                    checkedIcon={
                      <IoMdCheckboxOutline
                        className='text-[#F24E1E] text-2xl mb-[2px]' />
                    }
                  />}
                />
                <Button onClick={()=> dispatch(removeSavedResult(i))} className='p-1 min-w-fit m-0'>
